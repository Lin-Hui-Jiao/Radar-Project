 ...
 */
 ...
 /* CMD
 ./Q-VIEW --index ../mergeindex/indexes/test_area.3idx --start_lat 22.2806 --start_lon 114.1670 --start_h 100 --end_lat 22.2807 --end_lon 114.1675 --end_h 30
 */
#include <iostream>
#include <fstream>
#include <sstream>
...
// 从 line[0] 指向 line[1] 的这条有限线段，是否穿过了由 tri[0], tri[1], tri[2] 三点构成的三角形？
...
//viewshed二维数组里面包含了可视化分析的结果，1表示可见，0表示不可见
...
int main(int nArgc, char **papszArgv)
{
    ...
    for (int iArg = 1; iArg < nArgc; iArg++)
    {
        ...
        if (strcmp(papszArgv[iArg], "--start_lat") == 0)
        {
            y_start = atof(papszArgv[iArg + 1]);
        }
        if (strcmp(papszArgv[iArg], "--start_lon") == 0)
        {
            x_start = atof(papszArgv[iArg + 1]);
        }
        if (strcmp(papszArgv[iArg], "--start_h") == 0)
        {
            h_start = atof(papszArgv[iArg + 1]);
        }
        if (strcmp(papszArgv[iArg], "--end_lat") == 0)
        {
            y_end = atof(papszArgv[iArg + 1]);
        }
        if (strcmp(papszArgv[iArg], "--end_lon") == 0)
        {
            x_end = atof(papszArgv[iArg + 1]);
        }
        if (strcmp(papszArgv[iArg], "--end_h") == 0)
        {
            h_end = atof(papszArgv[iArg + 1]);
        }
    }
    ...
    RTree3d tree;
    tree.Load(index); //将整个 3D R-tree 空间索引结构加载到内存中
    ...
    PJ_COORD coord_start = proj_coord(y_start, x_start, 0, 0);
    PJ_COORD coord_end = proj_coord(y_end, x_end, 0, 0);
    PJ_COORD result_start = proj_trans(proj_from, PJ_FWD, coord_start);
    PJ_COORD result_end = proj_trans(proj_from, PJ_FWD, coord_end);
    y_start=result_start.xy.x-min_y;//没有错误，就是这样的
    y_end=result_end.xy.x-min_y;
    x_start=result_start.xy.y-min_x;
    x_end=result_end.xy.y-min_x;
    std::cout<<"x_start: "<<x_start<<" and y_start: "<<y_start<<std::endl;
    std::cout<<"x_end: "<<x_end<<" and y_end: "<<y_end<<std::endl;
    //获取起始点高度
    int height_start = tree.Getheight3d(indexRange_x + x_start, indexRange_y + y_start, HeightCallback) + 0.1;
    h_start+=height_start;//起始点实际高度
    //获取终点高度
    int height_end = tree.Getheight3d(indexRange_x + x_end, indexRange_y + y_end, HeightCallback) + 0.1;
    h_end+=height_end;//终点实际高度
    cout<<"h_start="<<h_start<<" "<<"h_end="<<h_end<<endl;
    //两点之间通视分析
    Rect3d search_rect(indexRange_x + x_start, indexRange_y +y_start, h_start, indexRange_x + x_end, indexRange_y +y_end, h_end); // 250*250 500*500
...
// viewshed query
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            if ((minX + radius == j) && (minY + radius == i)) //逻辑错误？if (j == radius && i == radius)
            {
                rowPointers[ii][4 * j] = 0;     // red
                rowPointers[ii][4 * j + 1] = 0; // green
                rowPointers[ii][4 * j + 2] = 0; // blue
